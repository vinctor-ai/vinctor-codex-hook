...
describe("slackClassifier — workspace reads → slack", () => {
  it("reference + korotovsky workspace reads", () => {
    for (const t of ["slack_list_channels", "slack_get_users", "slack_get_user_profile", "channels_list", "channels_me", "conversations_unreads", "users_search"]) {
      assert.deepEqual(slackClassifier(t, { limit: 10 }), m("read", "slack"), t);
    }
  });
  it("get_user_profile is workspace-coarse even with user_id", () => {
    assert.deepEqual(slackClassifier("slack_get_user_profile", { user_id: "U01ABCDEFGH" }), m("read", "slack"));
  });
});

describe("slackClassifier — channel reads → slack/<channel>", () => {
  it("channel present", () => {
    assert.deepEqual(slackClassifier("slack_get_channel_history", { channel_id: C, limit: 20 }), m("read", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_history", { channel_id: C }), m("read", `slack/${C}`));
    assert.deepEqual(slackClassifier("slack_get_thread_replies", { channel_id: C, thread_ts: "1700000000.1" }), m("read", `slack/${C}`));
  });
  it("channel missing → workspace fallback (reads tolerate it)", () => {
    assert.deepEqual(slackClassifier("conversations_history", {}), m("read", "slack"));
  });
});

describe("slackClassifier — sends → slack/<channel>", () => {
  it("each send form", () => {
    assert.deepEqual(slackClassifier("slack_post_message", { channel_id: C, text: "hi" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("slack_reply_to_thread", { channel_id: C, thread_ts: "1.2", text: "r" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("slack_add_reaction", { channel_id: C, timestamp: "1.2", reaction: "thumbsup" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_add_message", { channel_id: C, payload: "hi" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("reactions_add", { channel_id: C, timestamp: "1.2", emoji: "x" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("reactions_remove", { channel_id: C, timestamp: "1.2", emoji: "x" }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_join", { channel_id: C }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_leave", { channel_id: C }), m("send", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_mark", { channel_id: C, ts: "1.2" }), m("send", `slack/${C}`));
  });
  it("DM and human-form targets pass through as-is", () => {
    assert.deepEqual(slackClassifier("slack_post_message", { channel_id: "D01DMUSER", text: "x" }), m("send", "slack/D01DMUSER"));
    assert.deepEqual(slackClassifier("conversations_add_message", { channel_id: "#general", payload: "x" }), m("send", "slack/#general"));
    assert.deepEqual(slackClassifier("conversations_add_message", { channel_id: "@john", payload: "x" }), m("send", "slack/@john"));
  });
});

describe("slackClassifier — search", () => {
  it("filter_in_channel present → channel; absent → workspace", () => {
    assert.deepEqual(slackClassifier("conversations_search_messages", { search_query: "q", filter_in_channel: C }), m("read", `slack/${C}`));
    assert.deepEqual(slackClassifier("conversations_search_messages", { search_query: "q" }), m("read", "slack"));
  });
});
...
describe("slackClassifier — edge / unknown", () => {
  ...
  it("conversations_replies with channel present → read:slack/<channel>", () => {
    assert.deepEqual(slackClassifier("conversations_replies", { channel_id: C, thread_ts: "1.2" }), m("read", `slack/${C}`));
  });
...
describe("slack dispatch + resolve integration", () => {
  it("dispatchMcpClassifier routes mcp__slack__<tool> to slackClassifier", () => {
    assert.deepEqual(
      ...
      { kind: "Mapped", action: "read", resource: "slack" });
  });
  it("resolve maps an mcp__slack send via the classifier", () => {
    const r = resolve(slEvent("mcp__slack__slack_post_message", { channel_id: "C01AB2CD3EF", text: "hi" }) as any,
      { version: 1, rules: [] });
    assert.deepEqual(r, { kind: "Mapped", action: "send", resource: "slack/C01AB2CD3EF", source: "classifier" });
  });

...
describe("dispatchCli — subcommands", () => {
  ...
  it("explain mapped --json → resource pkg/npm/_", async () => {
    const o = cap();
    ...
    assert.equal(parsed.resource, "pkg/npm/_");
  });
  ...
  it("doctor → runs checks, --json emits a report, exit 0 when healthy", async () => {
    const o = cap();
    const goodCfg = JSON.stringify({
      hooks: { PreToolUse: [{ matcher: "Bash|apply_patch|Edit|Write|mcp__.*", hooks: [{ type: "command", command: "vinctor-codex-hook" }] }] },
    });
    const code = await dispatchCli({
      argv: ["node", "cli", "doctor", "--json"],
      ...baseDeps(
        { "/home/u/.codex/hooks.json": goodCfg },
        { env: { HOME: "/home/u", VINCTOR_ENDPOINT: "http://x", VINCTOR_AGENT_KEY: "aak_x", VINCTOR_GRANT_REF: "grt_x" } },
      ),
      stdout: o.write,
    ...
    assert.equal(code, 0);
    const report = JSON.parse(o.out.join(""));
    assert.equal(report.command, "doctor");
    assert.ok(report.checks.some((c: { name: string }) => c.name === "classifier-smoke"));
  });

  it("doctor exit 1 when a check fails (node too old)", async () => {
    const o = cap();
    const code = await dispatchCli({
      argv: ["node", "cli", "doctor"],
      ...baseDeps({}, { env: {} }),
      stdout: o.write,
      binPath: "/x/vinctor-codex-hook",
      nodeVersion: "v18.0.0",
    });
    assert.equal(code, 1);
    assert.match(o.out.join(""), /node-version/);
  });
});

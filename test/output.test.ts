...
import { allow, abstain, decision, denyFor } from "../src/output.js";
import { MalformedPayloadError, ServiceUnavailableError, ActionDeniedError } from "../src/errors.js";

describe("output formatter", () => {
  it("allow omits permissionDecisionReason", () => {
  ...
  it("decision wraps an envelope for serialization", () => {
    assert.deepEqual(decision(allow()), { emit: "decision", output: allow() });
  });

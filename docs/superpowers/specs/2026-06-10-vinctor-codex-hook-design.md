...
> **Historical design note — superseded wire details (2026-07-11):** Current
> Codex reserves `permissionDecision: "allow"` for rewrites with `updatedInput`;
> a service permit therefore continues with empty stdout. Codex 0.144.1 emitted
> `apply_patch` with its envelope in `tool_input.command`, not `input`. The
> implementation and README are the current contract; the original text below

...
export function slackClassifier(tool: string, input: Record<string, unknown>): ClassifierResult {
  ...
  switch (desc.scope) {
    case "workspace":
      return { kind: "Mapped", action, resource: "message/slack" };
    case "channel": {
      const target = strField(input.channel_id);
      if (target) return { kind: "Mapped", action, resource: `message/slack/${target}` };
      if (action === "send") return RBU; // never send to an ambiguous target
      return { kind: "Mapped", action: "read", resource: "message/slack" };
    }
    case "search": {
      ...
      return {
        kind: "Mapped",
        action: "read",
        resource: target ? `message/slack/${target}` : "message/slack",
      };
    }
  }
}

...
const fetchOK = (): typeof fetch => async () => new Response(JSON.stringify({ decision: "permit", audit_event_id: "evt_test" }), { status: 200 });
...
describe("handleEvent", () => {
  it("Mapped + permit → empty stdout continuation", async () => {
  ...
  it("Mapped + boundary id -> forwards X-Vinctor-Boundary-Id", async () => {
    ...
    const fakeFetch: typeof fetch = async (_url, init) => {
      ...
      return new Response(JSON.stringify({ decision: "permit", audit_event_id: "evt_test" }), { status: 200 });
    };
    const res = await handleEvent(
      { hook_event_name: "PreToolUse", tool_name: "Bash", tool_input: { command: "npm publish" } },
...
describe("handleEvent — apply_patch", () => {
  it("editing .env + permit → empty stdout; body is write:secret/env", async () => {
...
describe("handleEvent — WebFetch / WebSearch / file tools", () => {
  it("WebFetch external + permit → empty stdout; body is send:net/external/<host>", async () => {
    let body: Record<string, unknown> | null = null;
    const fakeFetch: typeof fetch = async (_u, init) => {
      body = JSON.parse(String(init!.body));
      return new Response(JSON.stringify({ decision: "permit", audit_event_id: "evt_test" }), { status: 200 });
  ...
  it("Read of .env + permit → empty stdout; body is read:secret/env", async () => {
    let body: Record<string, unknown> | null = null;
    const fakeFetch: typeof fetch = async (_u, init) => {
      body = JSON.parse(String(init!.body));
      return new Response(JSON.stringify({ decision: "permit", audit_event_id: "evt_test" }), { status: 200 });

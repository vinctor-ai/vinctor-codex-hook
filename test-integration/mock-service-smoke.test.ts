...
before(async () => {
  ...
  writeFileSync(CONFIG, JSON.stringify({ default_decision: "deny", permit: ["deploy:npm/package", "read:secret/env"] }));
...
describe("mock-service smoke (shared vinctor-core fixture)", { skip }, () => {
  it("permit response allows a mapped action (npm publish → deploy:npm/package)", () => {
    assert.equal(runCli(bash("npm publish")).decision, "allow");
  });
  ...
  it("optional X-Vinctor-Boundary-Id is forwarded as a header, not in the strict body", () => {
    ...
    assert.equal(runCli(bash("npm publish"), { VINCTOR_BOUNDARY_ID: "bnd_codex" }).decision, "allow");
  });

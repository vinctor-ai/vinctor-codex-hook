...
describe("githubClassifier — reads", () => {
  it("repo-scoped reads → read:github/<o>/<r>/<kind>", () => {
    assert.deepEqual(githubClassifier("get_file_contents", { ...OR, path: "README.md" }), m("read", "github/acme/api/contents"));
    assert.deepEqual(githubClassifier("list_commits", OR), m("read", "github/acme/api/contents"));
    assert.deepEqual(githubClassifier("list_branches", OR), m("read", "github/acme/api/contents"));
  ...
  it("global reads → github/_/<kind>", () => {
    ...
    assert.deepEqual(githubClassifier("search_code", { query: "x" }), m("read", "github/_/contents"));
  });
...
describe("githubClassifier — mutations", () => {
  it("writes", () => {
    assert.deepEqual(githubClassifier("create_or_update_file", { ...OR, path: "a", content: "x", message: "m", branch: "main" }), m("write", "github/acme/api/contents"));
    assert.deepEqual(githubClassifier("push_files", { ...OR, branch: "main", files: [], message: "m" }), m("write", "github/acme/api/contents"));
    assert.deepEqual(githubClassifier("create_branch", { ...OR, branch: "f" }), m("write", "github/acme/api/contents"));
  ...
  it("delete_file → delete", () => {
    assert.deepEqual(githubClassifier("delete_file", { ...OR, path: "x", message: "m", branch: "main" }), m("delete", "github/acme/api/contents"));
  });
...
describe("githubClassifier — secret scanning → github/<o>/<r>/secret (canon)", () => {
  it("maps to the repo's secret kind", () => {
    assert.deepEqual(githubClassifier("get_secret_scanning_alert", { ...OR, alertNumber: 1 }), m("read", "github/acme/api/secret"));
    assert.deepEqual(githubClassifier("list_secret_scanning_alerts", OR), m("read", "github/acme/api/secret"));
  });
  it("read with owner/repo missing falls back to unknown segments", () => {
    assert.deepEqual(githubClassifier("list_secret_scanning_alerts", {}), m("read", "github/_/secret"));
  });
});

describe("githubClassifier — canon operation-name aliases", () => {
  it("read aliases", () => {
    assert.deepEqual(githubClassifier("get_issue", { ...OR, issue_number: 7 }), m("read", "github/acme/api/issue"));
    assert.deepEqual(githubClassifier("get_pull_request", { ...OR, pull_number: 42 }), m("read", "github/acme/api/pr"));
    assert.deepEqual(githubClassifier("get_release", { ...OR, tag: "v1.2.0" }), m("read", "github/acme/api/release"));
  });
  it("update_issue → write:github/<o>/<r>/issue", () => {
    assert.deepEqual(githubClassifier("update_issue", { ...OR, issue_number: 7 }), m("write", "github/acme/api/issue"));
  });
  it("create_release / publish_release → deploy:github/<o>/<r>/release (externally effective)", () => {
    assert.deepEqual(githubClassifier("create_release", { ...OR, tag: "v1.2.0" }), m("deploy", "github/acme/api/release"));
    assert.deepEqual(githubClassifier("publish_release", { ...OR, tag: "v1.2.0" }), m("deploy", "github/acme/api/release"));
  });
});
...
describe("githubClassifier — edge / unknown", () => {
  ...
  it("repo-scoped READ tolerates missing owner/repo (fallback resource)", () => {
    assert.deepEqual(githubClassifier("list_commits", {}), m("read", "github/_/contents"));
    assert.deepEqual(githubClassifier("list_commits", { owner: "acme" }), m("read", "github/acme/_/contents"));
  });
...
describe("github dispatch + resolve integration", () => {
  ...
  it("resolve maps an mcp__github mutation via the classifier", () => {
    const r = resolve(ghEvent("mcp__github__create_or_update_file",
      ...
      { version: 1, rules: [] });
    assert.deepEqual(r, { kind: "Mapped", action: "write", resource: "github/acme/api/contents", source: "classifier" });
  });

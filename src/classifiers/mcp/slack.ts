...
export function slackClassifier(tool: string, input: Record<string, unknown>): ClassifierResult {
  ...
  switch (desc.scope) {
    case "workspace":
      return { kind: "Mapped", action, resource: "slack" };
    case "channel": {
      const target = strField(input.channel_id);
      if (target) return { kind: "Mapped", action, resource: `slack/${target}` };
      if (action === "send") return RBU; // never send to an ambiguous target
      return { kind: "Mapped", action: "read", resource: "slack" };
    }
    case "search": {
      const target = strField(input.filter_in_channel);
      return { kind: "Mapped", action: "read", resource: target ? `slack/${target}` : "slack" };
    }
  }
}

  ...
  server = createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      if ((lastBody as { resource?: string }).resource === "npm/package") {
        res.writeHead(200).end(JSON.stringify({ decision: "permit" }));
      } else {
...
describe("service-backed E2E (local mock /v1/enforce)", () => {
  it("permit path: mapped + in-scope → allow", async () => {
    mode = "normal";
    ...
    assert.equal(res.output.hookSpecificOutput.permissionDecision, "allow");
  });

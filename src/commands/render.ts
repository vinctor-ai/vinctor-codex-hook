...
const DOCTOR_GLYPH: Record<DoctorReport["checks"][number]["status"], string> = {
  pass: "✓",
  warn: "⚠",
  fail: "✗",
};
...
export function renderDoctorText(report: DoctorReport): string {
  const lines = ["vinctor-codex-hook doctor", ""];
  for (const c of report.checks) {
    lines.push(`${DOCTOR_GLYPH[c.status]} ${c.name}: ${c.detail}`);
  }
  lines.push("");
  lines.push(report.ok ? "Result: no blocking problems." : "Result: ✗ blocking problem(s) above.");

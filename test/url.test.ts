...
describe("parseAndClassifyUrl — host classification", () => {
  ...
  it("classifies the reserved *.internal TLD as internal", () => {
    assert.equal(parseAndClassifyUrl("http://orders-api.internal/status").scope, "internal");
    assert.equal(parseAndClassifyUrl("http://db.svc.INTERNAL/x").scope, "internal");
  });

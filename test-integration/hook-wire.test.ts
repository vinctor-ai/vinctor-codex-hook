...
// decision (empty-output permit, or deny:action_denied) — never a fail-closed code, which
...
describe("integration: full hook path wire", { skip: !haveEnv }, () => {
  it("a mapped Bash command reaches the service and gets a real decision", async () => {
    ...
    if (res.emit === "abstain") return; // mapped call + successful service permit
    const { permissionDecision, permissionDecisionReason } = res.output.hookSpecificOutput;
    const reachedService = permissionDecisionReason === "Denied by Vinctor authorization: action_denied.";
    assert.ok(reachedService, `expected permit or action_denied, got ${permissionDecision} / ${permissionDecisionReason}`);

...
describe("enforce client", () => {
  it("sends POST /v1/enforce with X-Agent-Key and the strict body", async () => {
    ...
    const fakeFetch = mockFetch(calls, () => new Response(JSON.stringify({ decision: "permit" }), { status: 200 }));
  ...
  it("returns void on 200 permit", async () => {
    const fakeFetch = mockFetch([], () => new Response(JSON.stringify({ decision: "permit" }), { status: 200 }));

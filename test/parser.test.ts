...
describe("parser — apply_patch", () => {
  it("reads the current Codex tool_input.command field", () => {
    const p = parseEvent({
      hook_event_name: "PreToolUse",
      tool_name: "apply_patch",
      tool_input: { command: "*** Begin Patch\n*** Add File: package.json\n+{}\n*** End Patch" },
    });
    assert.equal(p.tool, "apply_patch");
    if (p.tool === "apply_patch") assert.equal(p.ops[0]?.normalizedPath, "package.json");
  });

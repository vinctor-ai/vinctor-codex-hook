...
describe("filesystemClassifier — writes / deletes", () => {
  ...
  it("delete_file / delete_directory / remove_directory → delete:fs/<path>", () => {
    // remove_directory is the spec-canonical name; delete_directory is the
    // reference-server-fork alias — both bind to the same canon operation.
    for (const t of ["delete_file", "delete_directory", "remove_directory"]) {

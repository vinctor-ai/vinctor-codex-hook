...
// The mock service permits resource "pkg/npm/_", denies everything else, and
...
before(async () => {
  server = createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      if ((lastBody as { resource?: string }).resource === "pkg/npm/_") {
...
describe("service-backed E2E (local mock /v1/enforce)", () => {
  ...
  it("strict enforce body + X-Agent-Key header on the live path", () => {
    ...
    assert.equal((lastBody as { resource: string }).resource, "pkg/npm/_");

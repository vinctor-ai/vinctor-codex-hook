...
| `log-wrapper.mjs` | Catch-all `PreToolUse` hook used **only during measurement**. Logs the emitted `tool_name` (one JSON line each) and returns `allow` so the agent proceeds. It is **not** the real hook — it does not classify, map, or call `/v1/enforce`. |
